...
author: 'Gerardo Vargas Dominguez'
format:
  html:
...
```{r}
library(dplyr)
sunapee <- filter(ice_data, lake_name == "Sunapee")

#ice_off_doy was being read as a character and was messing with
#the graphs that were being made so I made it numeric

sunapee$ice_off_doy <- as.numeric(sunapee$ice_off_doy)
```
...
ggplot(sunapee, aes(x= year, y = ice_off_doy)) +
  ...
  geom_line() +
  theme_classic() +
  labs(title = "Lake Sunapee Ice-Off Dates", x = "Year", y = "Day of the Year")
```
...
I expect the sign to be minus for the slope of the relationship since the graph created for question 1 shows a negative slope as the years go by.
...
The direction of the slope is negative. This matches the expectations
from question 2.
...
The R2 for the regression is 0.02055. Based on the R2, 'year' and 'ice_off_doy' are not strongly related. The R2 shows that they
...
sunapee <- add_predictions(sunapee, fit, var = "pred")
sunapee <- add_residuals(sunapee, fit, var = "resid")
```
...
ggplot(sunapee, aes(x = resid)) +
  ...
  theme_classic() +
  labs(title = "Lake Sunapee Residuals", x = "Residuals", y = "Count")
```

I believe that the residuals histogram do not meet the assumptions of s of the linear regression, show a bias, and are not normally distributed even though the data is centered around 0. The distribution of the data is not even and seems to be biased towards the right side of the data too much. 
...
Based on the 'summary()' above, the p-value for the slope is 2.5e-09.
...
Based on a slope value of -0.106, I believe that it would take about
66 more years for Lake Sunapee ice-off to shift by a week. I believe that this rate of change is very small and proves to be meaningless since it would take a long time to notice the small difference. 
...
I am not comfortable with proceeding with using this linear model for additional analysis and prediction. I believe that after residuals histogram does not meet the assumptions of the linear regression and it shows that there is a strong bias since the spread is abnormal. Something that was also observed was an R2 close to 0 meaning that there was a weak relationship between the variables that are being used. 
...
```{r}
prediction_2040 <- coef(fit)[1] + coef(fit)[2] * 2040
prediction_2040

quantile(sunapee$resid, probs = c(0.05,0.95))

lower_2040pred <- prediction_2040 + quantile(sunapee$resid, 0.05)
higher_2040pred <- prediction_2040 + quantile(sunapee$resid, 0.95)
lower_2040pred
higher_2040pred

confidence_interval_width <- higher_2040pred - lower_2040pred
confidence_interval_width

```
11.a: The ice-off day of the year 2040 is roughly expected to be on day 103 or day 102.6622.

11.b the 95% and 5% confidence interval is 14.15137 and -19.42732.

11.c: The 90% confidence interval is roughly between day 84 and day 117, or exactly on day 83.23486 and day 33.57868.

11.d: The exact width of the confidence interval is 33.57868 days or about 34 days.
...
```{r}
sunapee_lessthan <- filter(sunapee, year < 1970)
fit_lessthan <- lm(ice_off_doy ~ year, data = sunapee_lessthan)
fit_lessthan$coefficients[2]

sunapee_greaterthan <- filter(sunapee, year >= 1970)
fit_greaterthan <- lm(ice_off_doy ~ year, data = sunapee_greaterthan)
fit_greaterthan$coefficients[2]

ggplot(sunapee, aes(x= year, y = ice_off_doy)) +
  geom_point() +
  geom_line() +
  geom_abline(intercept = coef(fit_greaterthan)[1],
  slope = coef(fit_greaterthan)[2], 
  color = "red",
  linewidth = 1
  ) +
  geom_abline(intercept = coef(fit_lessthan)[1],
  slope = coef(fit_lessthan)[2], 
  color = "blue",
  linewidth = 1
  ) +
  theme_classic() +
  labs(title = "Lake Sunapee Ice-Off Trend Before and After 1970", subtitle = "Blue Line:Before 1970     Red Line:After 1970", x = "Year", y = "Day of the Year")
```

The ice-off day of year has been changing quicker in recent years at Lake Sunapee. By splitting the Lake Sunapee data in two, data before 1970 and data after 1970, we can see that the slope shown in the data that is less than the year 1970 is less than the slope for the years greater than 1970. The slope found when analyzing the data before the year 1970 was -0.109668 while the slope after the year 1970 is -0.3023923. Just based on the slope we can see the difference in change rate has been increasing in the most recent years. To visualize this both of the slopes were placed on the graph created for question 1 to see the difference in steepness between the slopes. On the graph the blue line represent the slope for the data representing the years before 1970 and the red line represent the slope for the data representing the years after 1970. The graph shows the difference between the two slopes and may help understand the major difference in the rate of change between the two sets of data.
...
ChatGPT was used for question 10 and the following prompt was asked
'how do I use geom_abline if I have an intercept and slope'. The following sintax was provided by chat gpt 'geom_abline(intercept = <intercept_value>, slope = <slope_value>)' and is what was used to generate the graph for question 10. Afterwards the following prompt was asked, 'can I change the color and size of the line?' and chat gpt responded with 'Absolutely! With geom_abline(), you can easily change both the color and the thickness (size) of the line using the color and size arguments. You can also change the line type with linetype.'. This was then used to modify the line visibility to improve the graph with R giving me an error after using size argument and letting me know that it has been switched to linewidth.

ChatGPT was used on question 12 and was asked 'is there any way to add a subtitle in ggplot?'. ChatGPT answered library(ggplot2)
# Example dataset
df <- data.frame(
  x = 1:10,
...
ggplot(df, aes(x = x, y = y)) +
  ...
  labs(
    title = "Example of a Plot with labs()",
    subtitle = "Shows points and a connecting line",
    x = "Independent Variable (X)",
    y = "Dependent Variable (Y)",
    caption = "Source: Simulated data for demonstration",
    tag = "Figure 1"
  ) +
  theme_minimal()
